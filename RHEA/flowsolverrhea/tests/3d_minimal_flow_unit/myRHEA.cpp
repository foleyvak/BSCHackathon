...
const double Re_c       = 3000.0;				/// Centerline Reynolds number
const double Ma         = 3.0e-1;				/// Mach number
//const double Pr         = 0.71;					/// Prandtl number
const double rho_0      = 1.0;					/// Reference density	
const double U_c        = 1.0;					/// Centerline velocity
const double mu         = rho_0*U_c*delta/Re_c;			/// Dynamic viscosity	
const double nu         = mu/rho_0;				/// Kinematic viscosity	
//const double kappa      = c_p*mu/Pr;				/// Thermal conductivity
const double Re_tau     = 137.0; 				/// Friction Reynolds (approximated)
const double u_tau      = mu*Re_tau/(rho_0*delta);		/// Friction velocity (approximated)
const double tau_w      = rho_0*u_tau*u_tau;			/// Wall shear stress (approximated)
const double P_0        = rho_0*U_c*U_c/( gamma_0*Ma*Ma );	/// Reference pressure
//const double T_0        = P_0/( rho_0*R_specific );		/// Reference temperature
//const double L_x        = 1.89*delta;				/// Streamwise length
const double L_y        = 2.0*delta;				/// Wall-normal height
const double L_z        = 3.78*delta;				/// Spanwise width
const double V_dot      = (2.0/3.0)*U_c*L_y*L_z;		/// Volumetric flow rate based on laminar equivalent system	
...
/// const double time_step  = 1.0e-3;                               /// Fixed time step value [s]
    ...
    for(int i = topo->iter_common[_ALL_][_INIX_]; i <= topo->iter_common[_ALL_][_ENDX_]; i++) {
        for(int j = topo->iter_common[_ALL_][_INIY_]; j <= topo->iter_common[_ALL_][_ENDY_]; j++) {
            for(int k = topo->iter_common[_ALL_][_INIZ_]; k <= topo->iter_common[_ALL_][_ENDZ_]; k++) {
                ...
                y_dist              = min( y_field[I1D(i,j,k)], 2.0*delta - y_field[I1D(i,j,k)][j] );
...
void myRHEA::calculateSourceTerms() {
    ...
    /// Evaluate numerical volumetric flow rate

    /// Calculate local values
    double local_sum_u_volume = 0.0;
    double local_sum_volume   = 0.0;

    /// Inner: u & volume
    double delta_x, delta_y, delta_z, volume;
    #pragma acc parallel loop collapse(3) reduction(+:local_sum_vol_u) reduction(+:local_sum_vol) present(this, u_field.vector[0:_ls_], x_field.vector[0:_ls_],y_field.vector[0:_ls_], z_field.vector[0:_ls_])
    for(int i = topo->iter_common[_INNER_][_INIX_]; i <= topo->iter_common[_INNER_][_ENDX_]; i++) {
        for(int j = topo->iter_common[_INNER_][_INIY_]; j <= topo->iter_common[_INNER_][_ENDY_]; j++) {
            for(int k = topo->iter_common[_INNER_][_INIZ_]; k <= topo->iter_common[_INNER_][_ENDZ_]; k++) {    
                ...
                /// Sum values
                local_sum_u_volume += u_field[I1D(i,j,k)]*volume;
                local_sum_volume   += volume;
            }
        }
    }
    ...
    double global_sum_volume;
    MPI_Allreduce(&local_sum_volume, &global_sum_volume, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    double global_u_b = global_sum_u_volume/global_sum_volume;

    /// Calculate volumetric flow rate
    double V_dot_numerical = global_u_b*L_y*L_z;

    /// Update controller variables
    controller_error   = V_dot - V_dot_numerical;
