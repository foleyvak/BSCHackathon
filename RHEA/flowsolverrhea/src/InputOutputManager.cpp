...
void WriteReadHDF5::write2dDataOutputSlice(DistributedArray &x_field, DistributedArray &y_field, DistributedArray &z_field, ComputationalDomain* mesh, ParallelTopology* topo, char const* outname_dos, char const* dos_normal, double dos_x_position_adjusted, double dos_y_position_adjusted, double dos_z_position_adjusted, bool dos_gen_xdmf, int it) {
    ...
    sprintf(filename,"%s_%d.h5",outname_dos,it);
    ...
    int num_dims_2dPlane = 3;
    hsize_t dim_gsize_2dPlane[num_dims_2dPlane];
    hsize_t dim_lsize_2dPlane[num_dims_2dPlane];
    hsize_t dim_offset_2dPlane[num_dims_2dPlane];
    double *slab_vec_2dPlane = NULL;
    if( strcmp( dos_normal, "x_normal" ) == 0 ) {
        dim_gsize_2dPlane[0] = (topo->getMesh())->getGNz() + 2;
        dim_gsize_2dPlane[1] = (topo->getMesh())->getGNy() + 2;
        ...
        if( plane_contained ) {
            dim_lsize_2dPlane[0] = topo->lenslabz;
            dim_lsize_2dPlane[1] = topo->lenslaby;
            dim_lsize_2dPlane[2] = 1;
        } else {
            dim_lsize_2dPlane[0] = 0;
            dim_lsize_2dPlane[1] = 0;
            dim_lsize_2dPlane[2] = 0;
        }
        dim_offset_2dPlane[0] = topo->offslab_z;
        dim_offset_2dPlane[1] = topo->offslab_y;
        dim_offset_2dPlane[2] = 0;
        slab_vec_2dPlane = new double[( topo->lenslabz )*( topo->lenslaby )];
    } else if( strcmp( dos_normal, "y_normal" ) == 0 ) {
        dim_gsize_2dPlane[0] = (topo->getMesh())->getGNz() + 2;
        dim_gsize_2dPlane[1] = 1;
        ...
        if( plane_contained ) {
            dim_lsize_2dPlane[0] = topo->lenslabz;
            dim_lsize_2dPlane[1] = 1;
            dim_lsize_2dPlane[2] = topo->lenslabx;
        } else {
            dim_lsize_2dPlane[0] = 0;
            dim_lsize_2dPlane[1] = 0;
            dim_lsize_2dPlane[2] = 0;
        ...
        dim_offset_2dPlane[0] = topo->offslab_z;
        dim_offset_2dPlane[1] = 0;
        dim_offset_2dPlane[2] = topo->offslab_x;
        slab_vec_2dPlane = new double[( topo->lenslabz )*( topo->lenslabx )]; 
    } else if( strcmp( dos_normal, "z_normal" ) == 0 ) {
        dim_gsize_2dPlane[0] = 1;
        dim_gsize_2dPlane[1] = (topo->getMesh())->getGNy() + 2;
        dim_gsize_2dPlane[2] = (topo->getMesh())->getGNx() + 2;
        bool plane_contained = false;
        for(int k =  myTopo->iter_slab[_INIZ_]; k <=  myTopo->iter_slab[_ENDZ_]; k++){
        ...
        dim_offset_2dPlane[0] = 0;
        dim_offset_2dPlane[1] = topo->offslab_y;
        dim_offset_2dPlane[2] = topo->offslab_x;
        slab_vec_2dPlane = new double[( topo->lenslaby )*( topo->lenslabx )]; 
    }
    hsize_t stride_2dPlane[num_dims_2dPlane];
    stride_2dPlane[0] = 1;
    stride_2dPlane[1] = 1;
    stride_2dPlane[2] = 1;
    hsize_t block_2dPlane[num_dims_2dPlane];
    block_2dPlane[0] = 1;
    block_2dPlane[1] = 1;
    block_2dPlane[2] = 1;

    hid_t filespace_id = H5Screate_simple(num_dims_2dPlane, dim_gsize_2dPlane, NULL); /*global data space*/
    hid_t memspace_id  = H5Screate_simple(num_dims_2dPlane, dim_lsize_2dPlane, NULL); /*local data space*/
     
    for(std::list<DistributedArray*>::iterator it=fieldList.begin();it!=fieldList.end();++it) {

        hid_t dataset_id1 = H5Dcreate2(file_id,(*it)->printName(), H5T_NATIVE_DOUBLE, filespace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

        status = H5Sselect_hyperslab(filespace_id,H5S_SELECT_SET,dim_offset_2dPlane,stride_2dPlane,dim_lsize_2dPlane,block_2dPlane);

        int l=0;
        for(int k = myTopo->iter_slab[_INIZ_]; k <= myTopo->iter_slab[_ENDZ_]; k++)
            for(int j =  myTopo->iter_slab[_INIY_]; j <=  myTopo->iter_slab[_ENDY_]; j++)
                for(int i =  myTopo->iter_slab[_INIX_]; i <=  myTopo->iter_slab[_ENDX_]; i++){
                    if( ( strcmp( dos_normal, "x_normal" ) == 0 ) && ( abs( x_field[I1D(i,j,k)] - dos_x_position_adjusted ) < ( 1.1*numeric_limits<double>::min() ) ) ) {
                        slab_vec_2dPlane[l] = (*it)->vector[I1D(i,j,k)];
                        l=l+1;
                    } else if( ( strcmp( dos_normal, "y_normal" ) == 0 ) && ( abs( y_field[I1D(i,j,k)] - dos_y_position_adjusted ) < ( 1.1*numeric_limits<double>::min() ) ) ) {
                        slab_vec_2dPlane[l] = (*it)->vector[I1D(i,j,k)];
                        l=l+1;
                    } else if( ( strcmp( dos_normal, "z_normal" ) == 0 ) && ( abs( z_field[I1D(i,j,k)] - dos_z_position_adjusted ) < ( 1.1*numeric_limits<double>::min() ) ) ) {
                        slab_vec_2dPlane[l] = (*it)->vector[I1D(i,j,k)];
                        l=l+1;
                    }
                }

        status = H5Dwrite(dataset_id1, H5T_NATIVE_DOUBLE, memspace_id, filespace_id, fa_plist_id2, slab_vec_2dPlane);
   ...
   if( dos_gen_xdmf ) {
	   if(myTopo->getRank() == 0 ) {
		   ...
		   sprintf(filename2,"%s_%d.xdmf",outname_dos,it);
		   sprintf(filename3,"%s_%d",outname_dos,it);
		   ...
		   outfile << "  <Domain> " << endl;
		   outfile << "    <Grid Name='"<<filename3<<"' GridType='Uniform'>" << endl;
		   outfile << "      <Topology TopologyType='3DSMesh' Dimensions='"<<dim_gsize_2dPlane[0]<<" "<<dim_gsize_2dPlane[1]<<" "<<dim_gsize_2dPlane[2]<<"'/>" << endl;
		   outfile << "      <Geometry GeometryType='X_Y_Z'>" << endl;
		   outfile << "        <DataItem Name='x' Dimensions='"<<dim_gsize_2dPlane[0]<<" "<<dim_gsize_2dPlane[1]<<" "<<dim_gsize_2dPlane[2]<<"' NumberType='Float' Precision='8' Format='HDF'>" << endl;
		   outfile << "            "<<filename<<":/x" << endl;
		   outfile << "        </DataItem>" << endl;
		   outfile << "        <DataItem Name='y' Dimensions='"<<dim_gsize_2dPlane[0]<<" "<<dim_gsize_2dPlane[1]<<" "<<dim_gsize_2dPlane[2]<<"' NumberType='Float' Precision='8' Format='HDF'>" << endl;
		   outfile << "            "<<filename<<":/y" << endl;
		   outfile << "        </DataItem>" << endl;
		   outfile << "        <DataItem Name='z' Dimensions='"<<dim_gsize_2dPlane[0]<<" "<<dim_gsize_2dPlane[1]<<" "<<dim_gsize_2dPlane[2]<<"' NumberType='Float' Precision='8' Format='HDF'>" << endl;
		   outfile << "            "<<filename<<":/z" << endl;
		   outfile << "        </DataItem>" << endl;
		   outfile << "      </Geometry>" << endl;
		   for(std::list<DistributedArray*>::iterator it=fieldList.begin();it!=fieldList.end();++it) {
			   if( strcmp((*it)->printName(),"x") != 0 && strcmp((*it)->printName(),"y") != 0 && strcmp((*it)->printName(),"z") != 0 ) {
				   outfile << "      <Attribute Name='"<<(*it)->printName()<<"' AttributeType='Scalar' Center='Node'>" << endl;
				   outfile << "        <DataItem Dimensions='"<<dim_gsize_2dPlane[0]<<" "<<dim_gsize_2dPlane[1]<<" "<<dim_gsize_2dPlane[2]<<"' NumberType='Float' Precision='8' Format='HDF'>" << endl;
				   outfile << "          "<<filename<<":/"<<(*it)->printName() << endl;
				   outfile << "        </DataItem>" << endl;
				   outfile << "      </Attribute>" << endl;
			   }
