...
#endif
        int* p = nullptr; *p = 42;	// Causes segmentation fault
    }
...
CoolPropTransportCoefficients::CoolPropTransportCoefficients() : BaseTransportCoefficients() {};
        
CoolPropTransportCoefficients::CoolPropTransportCoefficients(const string configuration_file) : BaseTransportCoefficients(configuration_file) {
...
CoolPropTransportCoefficients::~CoolPropTransportCoefficients() {
...
void CoolPropTransportCoefficients::readConfigurationFile() {
...
double CoolPropTransportCoefficients::calculateDynamicViscosity(const double &P, const double &T, const double &rho) {
    ...
    return( cp_substance->viscosity() );

};
        
double CoolPropTransportCoefficients::calculateThermalConductivity(const double &P, const double &T, const double &rho) {

    /// CoolProp requires state update before extracting properties
    /// PT_INPUTS specifies that inputs are pressure (P) and temperature (T)
    cp_substance->update(CoolProp::PT_INPUTS, P, T);

    return( cp_substance->conductivity() );
...
SvdSurrogateTransportCoefficients::SvdSurrogateTransportCoefficients() : BaseTransportCoefficients() {};
        
SvdSurrogateTransportCoefficients::SvdSurrogateTransportCoefficients(const string configuration_file) : BaseTransportCoefficients(configuration_file) {
...
SvdSurrogateTransportCoefficients::~SvdSurrogateTransportCoefficients(){
		
    delete[] substance_P_vector;	
    delete[] substance_T_vector;
    //delete[] substance_rho_vector;	
    //delete[] substance_e_vector;    
    delete[] substance_T_P_to_mu_matrix_U;	
    delete[] substance_T_P_to_mu_matrix_S;	
    delete[] substance_T_P_to_mu_matrix_VT;	
    delete[] substance_T_P_to_kappa_matrix_U;	
    delete[] substance_T_P_to_kappa_matrix_S;	
    delete[] substance_T_P_to_kappa_matrix_VT;	
		
};

void SvdSurrogateTransportCoefficients::readConfigurationFile() {
	...
	/// Allocate matrices data
        ...
        substance_T_P_to_mu_matrix_U = new double[substance_SVD_rank*substance_SVD_rank];
        substance_T_P_to_mu_matrix_S = new double[substance_SVD_rank];
        substance_T_P_to_mu_matrix_VT = new double[substance_SVD_rank*substance_SVD_rank];
        substance_T_P_to_kappa_matrix_U = new double[substance_SVD_rank*substance_SVD_rank];
        substance_T_P_to_kappa_matrix_S = new double[substance_SVD_rank];
        substance_T_P_to_kappa_matrix_VT = new double[substance_SVD_rank*substance_SVD_rank];
	...
	for(int d = 0; d < (substance_SVD_rank*substance_SVD_rank); ++d) {
            substance_T_P_to_mu_matrix_U[d] = substance_SVD_data["substance_T_P_to_mu_matrix_U"][d].as<double>(); 
            substance_T_P_to_mu_matrix_VT[d] = substance_SVD_data["substance_T_P_to_mu_matrix_VT"][d].as<double>();
            substance_T_P_to_kappa_matrix_U[d] = substance_SVD_data["substance_T_P_to_kappa_matrix_U"][d].as<double>(); 
            substance_T_P_to_kappa_matrix_VT[d] = substance_SVD_data["substance_T_P_to_kappa_matrix_VT"][d].as<double>();	    
	}
	for(int d = 0; d < substance_SVD_rank; ++d) {
            ...
            substance_T_P_to_mu_matrix_S[d] = substance_SVD_data["substance_T_P_to_mu_matrix_S"][d].as<double>();
            substance_T_P_to_kappa_matrix_S[d] = substance_SVD_data["substance_T_P_to_kappa_matrix_S"][d].as<double>();
	}
...
double SvdSurrogateTransportCoefficients::calculateDynamicViscosity(const double &P, const double &T, const double &rho) {
    ...
    int vt_row = -1;
    double dynamic_viscosity = 0.0, u_k = 0.0, vt_k = 0.0;
    for( int k = 0; k < substance_SVD_rank; ++k ) {
        /// Mode k for temperature from U (row i and row i+1)
        u_k = ( 1.0 - w_T )*substance_T_P_to_mu_matrix_U[u_row1 + k] + w_T*substance_T_P_to_mu_matrix_U[u_row2 + k];
        /// Mode k for pressure from VT (row k, column j and j+1)
        vt_row = k*substance_SVD_rank;
        vt_k = ( 1.0 - w_P )*substance_T_P_to_mu_matrix_VT[vt_row + j] + w_P*substance_T_P_to_mu_matrix_VT[vt_row + j + 1];
        /// Update dynamic viscosity value
        dynamic_viscosity += substance_T_P_to_mu_matrix_S[k]*u_k*vt_k;
    }
...
double SvdSurrogateTransportCoefficients::calculateThermalConductivity(const double &P, const double &T, const double &rho) {
    ...
    /// Accumulate interpolated SVD reconstruction across rank substance_SVD_rank
    int vt_row = -1;
    double thermal_conductivity = 0.0, u_k = 0.0, vt_k = 0.0;
    for( int k = 0; k < substance_SVD_rank; ++k ) {
        /// Mode k for temperature from U (row i and row i+1)
        u_k = ( 1.0 - w_T )*substance_T_P_to_kappa_matrix_U[u_row1 + k] + w_T*substance_T_P_to_kappa_matrix_U[u_row2 + k];
        /// Mode k for pressure from VT (row k, column j and j+1)
        vt_row = k*substance_SVD_rank;
        vt_k = ( 1.0 - w_P )*substance_T_P_to_kappa_matrix_VT[vt_row + j] + w_P*substance_T_P_to_kappa_matrix_VT[vt_row + j + 1];
        /// Update dynamic viscosity value
        thermal_conductivity += substance_T_P_to_kappa_matrix_S[k]*u_k*vt_k;
    }
